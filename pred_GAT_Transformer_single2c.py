...
def testModel(name, mode, XS, YS):
    ...
    MSE, RMSE, MAE, MAPE = Metrics.evaluate(YS, YS_pred)
    print('*' * 40)
    print("%s, %s, Torch MAE, %.10e, %.3f" % (name, mode, torch_score, torch_score))
    f = open(PATH + '/' + name + '_prediction_scores.txt', 'a')
    f.write("%s, %s, Torch MAE, %.10e, %.3f\n" % (name, mode, torch_score, torch_score))
...
################# Parameter Setting #######################
MODELNAME = 'Transformersingle2c_horizon_'+str(args.horizon)
KEYWORD = MODELNAME + '_' + TASK  +' _CHANNEL2_in+out_ADJ'  + str(args.adjdata) + datetime.datetime.now().strftime("%y%m%d%H%M")
PATH = './save/' + KEYWORD
...
def main():
    ...
    shutil.copy2('pred_GAT_Transformer_single2c.py', PATH)
