...
def getXSYS(data, mode):
    ...
    return XS, YS
# XS, YS shape is : samples * channel * (input_step or pred_step) * n_route   XS (1046, 2, 12, 81)   YS (1046, 1, 12, 81)
def getModel(name):
    ks, kt, bs, T, n, p = 3, 3, [[CHANNEL2, 16, 64], [64, 16, 64]], INPUT_STEP, N_NODE, 0
...
def trainModel(name, mode, XS, YS):
    ...
    model = getModel(name)
    summary(model, (CHANNEL2, INPUT_STEP, N_NODE), device=device)
...
################# Parameter Setting #######################
MODELNAME = 'STGCN2c_single'+str(args.horizon)
KEYWORD = MODELNAME + '_' + TASK +'_adj_' + args.adjdata +'_CHANNEL2_in+out_'  + DATANAME  + '_' + datetime.datetime.now().strftime("%y%m%d%H%M")
PATH = './save/' + KEYWORD
...
def main():
    ...
    shutil.copy2('pred_STGCN2c.py', PATH)
