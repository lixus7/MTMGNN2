...
import Metrics
from Utils import *
from DCRNN_GAT1 import *
from Param import *

CHANNEL = 3 
# data in (2608, 3, 24, 81)    dcrnn in summary(model, (INPUT_STEP, N_NODE, CHANNEL), device=device)   out   (B, horizon, N, C)
def getXSYS(data, mode):
    ...
    return XS, YS
# XS, YS shape is : samples * channel * (input_step or pred_step) * n_route   XS (1046, 12, 81, 3)   YS (1046, pred_step, 81, 1)
...
def getModel(name):
    ...
    W = torch.Tensor(W)    
    model = DCRNN(device, num_nodes=N_NODE, input_dim=CHANNEL, out_horizon= 1, P=W).to(device)
    return model
...
################# Parameter Setting #######################
MODELNAME = 'DCRNN_GAT1_single'+str(args.horizon)
...
# seq_data shape is : samples * channel * (input_step+pred_step) * n_route   (2608, 3, 24, 81)
